...
class BiModalStackedELMAE(object):
    ...
    def __init__(self, n_hidden1, n_hidden2, n_hidden3, n_hidden4, n_joint,
        ...
        for i in xrange(self.iteration):
            ...
            modal2.append(StackedELMAutoEncoder(n_hidden2, n_coef2, activation=activation))
            # Create CCA Object
            if joint_method in jm:
                if n_comp == None:
    ...
    def crossmodal_training(self, input1, input2, ccaobj):
        ...
        print 'NumData: %s' % d
        print "input1: %s, input2: %s" % (input1.shape[1], input2.shape[1])
        ...
        print 'max: ', np.max(np.array(corr)), '\nmin: ', np.min(np.array(corr)),\
            '\nmean:', np.mean(np.array(corr)), '\ncorr:', np.array(corr)


        #print  self.cca.eigvals[:512]

        #h_out1, h_out2 = x_c, y_c
        h_out1, h_out2 = sigmoid(x_c), sigmoid(y_c)
    ...
    def joint(self, input1, input2, method='pcca', save=False):
    ...
    def pre_train(self, input1, input2, filename=None):
        ...
        h_out = self.joint(h_out1, h_out2)
        np.save('joint_tra.npy', h_out)

        print '== classification layers'
        shared_rep = self.joint_layers.pre_train(h_out)
    ...
    def pre_extraction(self, input1, input2, limit=-1, filename=None):
        ...
        if input1 != None or input2 != None:
            ...
            elif self.joint_method == "cca" or self.joint_method == 'semi':
                #------ my cca code ( with regularization term )
